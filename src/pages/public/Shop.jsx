...
export default function Shop() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.category !== "All") {
      list = list.filter(
        (p) => p.category === filters.category.toLowerCase()
      );
    }
    ...
    if (filters.size) {
      list = list.filter((p) => p.sizes && p.sizes.includes(filters.size));
    }
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-obsidian-300">Loading products...</div>
      </div>
    );
  }
              ...
              <>
                ...
                >
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <button
                      onClick={() => setSidebar(false)}
                      className="text-obsidian-200 hover:text-white"
                    >
                      <svg
                        className="w-5 h-5"
                        fill="none"
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                ...
                {filters.tag !== "All" && (
                  <span className="flex items-center gap-2 text-xs border border-gold-500/40 text-gold-400 px-3 py-1.5">
                    {filters.tag}
                    <button
                      onClick={() => setFilters((p) => ({ ...p, tag: "All" }))}
                      className="hover:text-white"
                    >
                      ✕
                    </button>
