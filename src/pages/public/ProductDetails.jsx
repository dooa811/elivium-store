...
import { getById, getSimilar }  from "../../utils/products.js";
import { productReviews }       from "../../utils/reviews.js";
...
export default function ProductDetails() {
  ...
  const similar     = product ? getSimilar(id, product.category) : [];
  ...
  const handleAddToCart = () => {
    ...
    setSizeError(false);
    addToCart(product, size, product.colorNames[colorIdx], qty);
    setAdded(true);
        ...
        <div className="container-xl">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                ...
                {product.images.map((img, i) => (
                  <button key={i} onClick={() => setImgIdx(i)}
                    className={`overflow-hidden aspect-square border-2 transition-all duration-200 ${
                      imgIdx === i ? "border-gold-500" : "border-obsidian-600 hover:border-obsidian-400"
                    }`}>
            ...
            <div className="flex flex-col">
              <div className="mb-2">
                <span className="text-xs font-semibold uppercase tracking-widest text-gold-500">{product.category}</span>
                {product.isNew && <span className="ml-3 text-xs uppercase tracking-widest text-white bg-obsidian-600 px-2 py-0.5">New</span>}
              </div>
              <h1 className="font-display text-4xl sm:text-5xl font-bold text-white mb-4">{product.name}</h1>
              <div className="flex items-center gap-4 mb-6">
                <ProductRating rating={product.rating} reviews={product.reviews} size="lg" />
                <span className="text-obsidian-300 text-sm">{product.reviews} reviews</span>
              </div>
              ...
              <div className="flex items-center gap-4 mb-8">
                ...
                {product.discount > 0 && (
                  <>
                    <span className="text-obsidian-300 text-xl line-through">{formatPrice(product.originalPrice)}</span>
              ...
              <div className="mb-6">
                <p className="text-xs font-bold uppercase tracking-widest text-white mb-3">
                  Color: <span className="text-gold-400 font-normal">{product.colorNames[colorIdx]}</span>
                </p>
                <div className="flex gap-3">
                  {product.colors.map((c, i) => (
                    <button key={i} onClick={() => setColorIdx(i)} title={product.colorNames[i]}
                      className={`w-8 h-8 rounded-full border-2 transition-all duration-200 ${
                        colorIdx === i ? "border-gold-500 scale-110 shadow-gold-sm" : "border-obsidian-500 hover:border-obsidian-300"
                      }`}
                      style={{ background: c }}
                    />
                  ))}
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-3">
                  <p className="text-xs font-bold uppercase tracking-widest text-white">Size</p>
                  <button className="text-xs text-gold-500 hover:text-gold-300 underline underline-offset-2">Size Guide</button>
                </div>
                ...
                {sizeError && <p className="text-red-400 text-xs mt-2">Please select a size to continue.</p>}
              </div>

              {/* Qty */}
              <div className="flex items-center gap-4 mb-8">
                <p className="text-xs font-bold uppercase tracking-widest text-white">Quantity</p>
                <div className="flex items-center border border-obsidian-500">
                  <button onClick={() => setQty(q => Math.max(1, q - 1))}
                    className="w-10 h-10 flex items-center justify-center text-white hover:bg-obsidian-600 transition-colors text-lg">−</button>
              ...
              <div className="flex gap-3 mb-8">
                <Button onClick={handleAddToCart} variant={added ? "ghost" : "gold"} size="lg" className="flex-1">
                ...
                <button onClick={() => toggle(product)}
                  className={`w-14 h-14 flex items-center justify-center border transition-all duration-200 ${
                    wishlisted ? "border-gold-500 bg-gold-500 text-obsidian-900" : "border-obsidian-500 text-white hover:border-gold-500 hover:text-gold-400"
                  }`}>
              ...
              <div className="grid grid-cols-3 gap-3 py-6 border-t border-b border-obsidian-600 mb-6">
                {[
                  { icon: "🚚", label: "Free Shipping", sub: "Orders over $200" },
                  { icon: "↩", label: "Free Returns", sub: "Within 30 days"   },
                  { icon: "✦", label: "Authentic",     sub: "100% genuine"     },
                ].map(b => (
                  <div key={b.label} className="text-center">
                    <span className="text-2xl">{b.icon}</span>
                    <p className="text-xs font-semibold text-white mt-1">{b.label}</p>
                ...
                {["description", "details", "reviews"].map(t => (
                  <button key={t} onClick={() => setTab(t)}
                    className={`px-6 py-3 text-xs font-bold uppercase tracking-widest border-b-2 transition-all -mb-px ${
                      tab === t ? "border-gold-500 text-gold-400" : "border-transparent text-obsidian-300 hover:text-white"
                    }`}>
              ...
              <AnimatePresence mode="wait">
                ...
                {tab === "details" && (
                  <motion.div key="details" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    ...
                    {[
                      ...
                      <div key={d.label} className="flex justify-between border-b border-obsidian-700 pb-3">
                        <span className="text-obsidian-300 uppercase tracking-widest text-xs">{d.label}</span>
                ...
                {tab === "reviews" && (
                  <motion.div key="reviews" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    className="space-y-4">
                    {reviews.length === 0 && <p className="text-obsidian-300 text-sm">No reviews yet. Be the first!</p>}
                    {reviews.map(r => (
                      <div key={r.id} className="border-b border-obsidian-700 pb-4">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className="text-obsidian-300 text-xs ml-auto">{r.date}</span>
                        </div>
