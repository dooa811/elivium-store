...
export default function Cart() {
  ...
  if (items.length === 0) return (
    <div className="min-h-[70vh] flex flex-col items-center justify-center gap-6 px-4">
      ...
      <h2 className="font-display text-3xl font-bold text-white">Your cart is empty</h2>
      <p className="text-obsidian-200 font-light text-center">Discover our collection and add your favourite pieces.</p>
      <Link to="/shop"><Button variant="gold" size="lg">Start Shopping</Button></Link>
    </div>
  );
    ...
    <div className="min-h-screen">
      <div className="bg-obsidian-800 border-b border-obsidian-700 py-10 px-4">
        <div className="container-xl">
          ...
          <p className="text-obsidian-200 mt-2 font-light">{totalItems} item{totalItems !== 1 ? "s" : ""} in your cart</p>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-4">
              ...
              {items.map(item => (
                ...
                >
                  ...
                  <div className="col-span-12 sm:col-span-6 flex gap-4">
                    <Link to={`/product/${item.id}`} className="flex-shrink-0 w-24 h-28 overflow-hidden">
                    ...
                    <div className="flex-1 min-w-0">
                      <Link to={`/product/${item.id}`} className="text-white font-medium text-sm hover:text-gold-400 transition-colors line-clamp-2">
                        {item.name}
                      </Link>
                      <p className="text-obsidian-300 text-xs mt-1">Size: {item.size}</p>
                      <p className="text-obsidian-300 text-xs">Color: {item.color}</p>
                  ...
                  <div className="col-span-4 sm:col-span-2 text-center">
                    <span className="text-white text-sm font-semibold">{formatPrice(item.price)}</span>
                  </div>
                  ...
                  <div className="col-span-4 sm:col-span-2 flex items-center justify-center">
                    <div className="flex items-center border border-obsidian-500">
                      <button onClick={() => updateQty(item.key, item.qty - 1)}
                        className="w-8 h-8 flex items-center justify-center text-white hover:bg-obsidian-600 transition-colors">−</button>
                  ...
                  <div className="col-span-4 sm:col-span-2 text-right">
                    <span className="text-white font-semibold">{formatPrice(item.price * item.qty)}</span>
                  </div>
                </motion.div>
              ))}
            </AnimatePresence>
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <Link to="/shop" className="btn-outline-gold text-xs px-6 py-3 text-center">← Continue Shopping</Link>
            </div>
          </div>
            ...
            <div className="bg-obsidian-800 border border-obsidian-600 p-8 sticky top-28">
              ...
              <div className="space-y-4 border-b border-obsidian-600 pb-6 mb-6">
                <div className="flex justify-between text-sm">
                  <span className="text-obsidian-200">Subtotal ({totalItems} items)</span>
                  <span className="text-white font-semibold">{formatPrice(subtotal)}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-obsidian-200">Shipping</span>
                  <span className={shipping === 0 ? "text-gold-400 font-semibold" : "text-white font-semibold"}>
                    {shipping === 0 ? "Free" : formatPrice(shipping)}
                  </span>
                </div>
                {shipping > 0 && (
                  <p className="text-xs text-obsidian-300">Add {formatPrice(200 - subtotal)} more for free shipping</p>
                )}
              </div>
              ...
              <div className="flex justify-center gap-4 mt-5">
                {["💳 Visa", "💳 MC", "💰 COD"].map(p => (
                  <span key={p} className="text-[10px] text-obsidian-300">{p}</span>
                ))}
              </div>
            </div>
          </div>
